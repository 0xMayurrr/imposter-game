(function () {
      ...
      <section class="panel">
        <div class="grid two">
          <div>
            ...
            <p class="muted">Pass the device to the player below. They can check or change their name before looking.</p>
            <div class="field">
              <label>Your name</label>
              <input data-reveal-name-index="${round.revealIndex}" value="${esc(card.name)}" maxlength="20" placeholder="Player name" />
            </div>
            <div class="button-row">
              ${card.revealed ? `
                <button class="primary-button" data-action="next-reveal">${round.revealIndex === round.assignments.length - 1 ? "Start Discussion" : "Next Player"}</button>
                <button class="ghost-button" data-action="hide-role">Hide again</button>
              ` : `<p class="small muted">Touch the card when you are alone to see your secret.</p>`}
              <button class="ghost-button" data-action="go-offline">Back to Setup</button>
            </div>
          </div>
          <div style="cursor: pointer" data-action="${card.revealed ? "" : "show-role"}">
            ${card.revealed ? assignmentMarkup(card) : `
              <div class="info-card" style="min-height: 220px; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; border: 2px dashed var(--line);">
                <div class="eyebrow">Ready for</div>
                <h3 style="font-size: 32px; margin: 8px 0;">${esc(card.name)}</h3>
                <p class="muted">Touch and see the words</p>
              </div>
            `}
          </div>
        </div>
        ...
        <div class="grid two">
          <div>
            ...
            <div class="player-list">
              ${room.players.map((p) => `<div class="player-card"><strong>${esc(p.name)}${p.id === state.online.meId ? " (You)" : ""}</strong><span class="small muted">${p.id === room.hostId ? "Host" : "Player"}${p.ready ? " • Ready" : ""}</span></div>`).join("")}
            </div>
          </div>
          <div>
            <h2>Room settings</h2>
      ...
      <section class="panel">
        <div class="grid two">
          ...
          <div>
            <h2>Player status</h2>
            <div class="player-list">
              ${readyMap.map((p) => `<div class="player-card"><strong>${esc(p.name)}${p.id === state.online.meId ? " (You)" : ""}</strong><span class="${p.ready ? "status-good" : "muted"}">${p.ready ? "Ready" : "Reading clue"}</span></div>`).join("")}
            </div>
          </div>
        </div>
      </section>
  ...
  function assignmentMarkup(card) {
    ...
    let teammates = "";
    if (card.role === "imposter" && card.teammates && card.teammates.length > 1) {
      const names = card.teammates.filter(n => n !== card.name).join(", ");
      teammates = `<div class="info-card" style="margin-top: 12px; border-color: var(--bad);"><div class="eyebrow">Your teammates</div><p>${esc(names)}</p></div>`;
    }
    return `<div><div class="role-banner ${roleClass}">${card.role === "imposter" ? "Imposter" : "Crew"}</div><h3>${esc(card.name)}</h3>${clue}${teammates}</div>`;
  }
  ...
  function onClick(event) {
    ...
    if (action === "show-role") return revealOffline(true);
    if (action === "hide-role") return revealOffline(false);
    ...
    if (action === "rematch") return startRoom();
  }
  ...
  function onInput(event) {
    ...
    if (input.dataset.revealNameIndex !== undefined) {
      const idx = Number(input.dataset.revealNameIndex);
      const val = input.value.slice(0, 20);
      state.setup.players[idx] = val;
      if (state.offline) {
        state.offline.assignments[idx].name = val;
        state.offline.players[idx].name = val;
      }
      return render();
    }
  ...
  function revealOffline(show) {
    state.offline.assignments[state.offline.revealIndex].revealed = show;
    render();
  }
  ...
  function buildRound(names, settings) {
    ...
    const imposterSet = new Set(shuffle(players.map((p) => p.id)).slice(0, settings.imposterCount));
    const imposterNames = players.filter(p => imposterSet.has(p.id)).map(p => p.name);
    return {
      players,
      assignments: players.map((player) => ({
        ...
        teammates: imposterSet.has(player.id) ? imposterNames : [],
